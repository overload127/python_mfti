...
# -----------------------------------------------------------------------
# Получаем имя
name_cur_player = input("Введите Ваш никнейм: ")
# размеры экрана
window_width = 800
window_heigth = 600
# Список цветов.
...
name_text = canv.create_text(
    5, 5, text="Никнейм: "+name_cur_player, anchor=tk.NW)
point_text = canv.create_text(
    5, 25, text="Количество очков: " + str(count_points), anchor=tk.NW)
...
# Окно нашей программы закрыто.
# Выполняем загрузку таблицы из файла
table_players = dict()
try:
    with open('table_best_player.txt', 'r',  encoding='utf-8') as file:
        for line in file:
            name, point = line.split(":")
            table_players[name] = int(point)
except IOError:
    print("Таблица не найдена!")
    print("Создадим новую.")

# Ищем свой старый результат и заменяемего если текущий лучше
if name_cur_player in table_players:
    point_from_table = table_players[name_cur_player]
    if point_from_table < count_points:
        table_players[name_cur_player] = count_points
else:
    # Просто добавляем текущий результат
    table_players[name_cur_player] = count_points

# Превращаем словарь в список и сортируем его по убыванию
list_players = list(table_players.items())
list_players.sort(key=lambda ele: ele[1], reverse=True)

# Записываем файл
try:
    with open('table_best_player.txt', 'w',  encoding='utf-8') as file:
        for line in list_players:
            file.writelines(line[0]+":"+str(line[1])+"\n")
    print("Таблица записана успешно.")
except IOError:
    print("Ошибка при записи таблицы")
