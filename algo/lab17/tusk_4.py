...
def move(me, enemies, bullets, bonuses, m):
    ...
    # Блок кода отвечающий за движение
    if patch_direct == 0:
        m.right()
        if me['pos'][_X] >= game.WIDTH-game.TANK_RADIUS-game.TANK_SPEED:
            patch_direct = 1
    elif patch_direct == 1:
        m.down()
        if me['pos'][_Y] >= game.HEIGHT-game.TANK_RADIUS-game.TANK_SPEED:
            patch_direct = 2
    elif patch_direct == 2:
        m.left()
        if me['pos'][_X] <= game.TANK_RADIUS+game.TANK_SPEED:
            patch_direct = 3
    elif patch_direct == 3:
        m.up()
        if me['pos'][_Y] <= game.TANK_RADIUS+game.TANK_SPEED:
            patch_direct = 0
    else:
